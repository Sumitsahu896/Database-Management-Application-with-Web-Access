...
//---------------------------------------------------------------------------
        if (n.isLeaf) {                                                      // handle leaf node level
            if (n.nKeys < MAX) {                                             // current node is not full
                ...
                if (n == root) {
                    root = makeRoot(n, n.key[n.nKeys - 1], r);
                // at root => make a new root
                }
                else if(r != null) {
                    hasSplit = true;//Possibility of a internal split to occur
                }
...
//---------------------------------------------------------------------------
            ...
            if(hasSplit){
                ...
                if(n.nKeys<MAX) {
                    ...
                    //else
                    wedge(midKey,r,(Node)n,n.find(midKey),false);
                    hasSplit=false;
                }
                else {
                    var rt=splitI(n);
                    ...
                    if (key.compareTo (n.key [MID]) < 0) {
                        ...
                        //else
                        wedge(midKey, r, n, n.find(midKey), false);       // wedge (key, ref) pair into node n
                    }
                    else {
                        //if(n.nKeys<MID-1) wedge(key1, r, rt, rt.find(key1), false);
                        //else
                        wedge(midKey, r, rt, rt.find(midKey), false);    // wedge (key, ref) pair into node rt
                    }
                    r = rt;
                }
            }
